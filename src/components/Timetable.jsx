...
const Timetable = ({ onNavigate }) => {
  ...
  return (
    <div className="h-full flex flex-col min-h-0 gap-3 sm:gap-4 overflow-y-auto lg:overflow-hidden pb-6 lg:pb-0 pr-1 lg:pr-0">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between flex-shrink-0 gap-3 sm:gap-0">
        <div>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-text-primary mb-1">Academic Timetable</h1>
          <p className="text-text-muted text-sm sm:text-base">View your schedule and identify free time blocks</p>
        </div>
        <div className="flex items-center gap-2 self-start sm:self-auto">
          ...
          <motion.button
            ...
            className="px-3 py-1.5 sm:px-4 sm:py-2 bg-white/20 text-text-primary rounded-lg font-semibold text-xs sm:text-sm flex items-center gap-1.5 border border-white/20"
          >
      ...
      <div className="card-dark p-1.5 sm:p-2 border border-white/20 flex-shrink-0">
        <div className="flex gap-1.5 overflow-x-auto custom-scrollbar pb-1 sm:pb-0">
          {days.map((day) => (
            <motion.button
              ...
              className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg font-semibold text-xs sm:text-sm whitespace-nowrap transition-all ${
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-3 gap-3 sm:gap-4 flex-1 min-h-0">
        <motion.div
          initial={{ opacity: 0, x: -10 }}
          animate={{ opacity: 1, x: 0 }}
          className="lg:col-span-2 card-dark p-3 sm:p-4 border border-white/20 flex flex-col min-h-[400px] sm:min-h-[450px] lg:min-h-0 overflow-hidden shrink-0 lg:shrink"
        >
          <h2 className="text-base sm:text-lg font-bold text-text-primary mb-2 sm:mb-3 flex-shrink-0">{selectedDay}'s Schedule</h2>
                      ...
                      <div className={`flex justify-between h-full gap-2 ${isTall ? 'items-center' : 'items-start'}`}>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <h3 className={`font-bold text-text-primary truncate ${isCompact ? 'text-[11px] sm:text-xs' : 'text-xs sm:text-sm'}`}>{cls.subject}</h3>
                            {status === 'current' && (
                              <motion.div
                                animate={{ scale: [1, 1.2, 1] }}
                                transition={{ repeat: Infinity, duration: 2 }}
                                className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-accent-green flex-shrink-0"
                              />
                            )}
                          </div>

                          {!isCompact && (
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] sm:text-xs text-text-muted mt-0.5 sm:mt-1 min-w-0">
        ...
        <motion.div
          initial={{ opacity: 0, x: 10 }}
          animate={{ opacity: 1, x: 0 }}
          className="card-dark p-3 sm:p-4 border border-white/20 flex flex-col min-h-[250px] lg:min-h-0 overflow-hidden shrink-0 lg:shrink"
        >
          <div className="flex items-center gap-1.5 mb-2 flex-shrink-0">
            <Sparkles className="w-4 h-4 text-text-primary" />
            <h2 className="text-base sm:text-lg font-bold text-text-primary">Available Free Time</h2>
          </div>
